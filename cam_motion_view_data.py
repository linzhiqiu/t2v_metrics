...
for label_name in all_labels:
    ...
    print(f'\n\nLabel Name {label_name} Positives {pos_test}')
    datasets[label_name.split(".")[-1]] = [{"image": sample, "question": "", "question_id":0, "label": "Yes"} for sample in pos_test] + \
    [{"image": sample, "question": "", "question_id":0, "label": "No"} for sample in neg_test] 
    print(
...
for label_name in all_labels:
    ...
    print(f'\n\n Dataset Point {dataset[0:5]}')

    for member in datasets[label_name.split(".")[-1]]:
        member["question"] = dataset[0]['texts'][0]
    continue
...
for key in datasets.keys():
    with open(key + '.jsonl', 'w') as writefile:
        writefile.writelines([json.dumps(json_obj) + "\n" for json_obj in datasets[key]])

...
# CUDA_VISIBLE_DEVICES=2 python cam_save_training.py --balancing True --save_file camera_centric_balanced_trainset.json
# CUDA_VISIBLE_DEVICES=2 python cam_save_training.py --video_label_file video_labels/cam_motion-20250227_0326ground_and_camera/label_names.json --save_file camera_centric_all_trainset.json
# CUDA_VISIBLE_DEVICES=2 python cam_save_training.py --video_label_file video_labels/cam_motion-20250227_0326ground_and_camera/label_names.json --save_file camera_centric_all_balanced_trainset.json --balancing True
import argparse
import os
import sys
import t2v_metrics
...
parser.add_argument(
    "--balancing",
    type=bool,
    help="The flag to balance the dataset",
    default=False,
)
...
for label_name in all_labels:
    ...
    for scene_name in scene_labels_balanced:
        pos_to_neg_ratio.append(len(scene_labels[scene_name][label_name]["pos"]) / (len(scene_labels[scene_name][label_name]["neg"]) + 1e-6))
        pos_to_neg_ratio_test.append(len(scene_labels[scene_name][label_name]["pos_test"]) / (len(scene_labels[scene_name][label_name]["neg_test"]) + 1e-6))
    max_ratio = max(pos_to_neg_ratio) + 1e-6
    max_ratio_test = max(pos_to_neg_ratio_test) + 1e-6
    for scene_name in scene_labels_balanced:
...
for label_name in all_labels:
    ...
    if args.balancing:
        max_number = min(len(all_labels[label_name]["pos"]), len(all_labels[label_name]["neg"]))
        all_labels[label_name]["pos"] = all_labels[label_name]["pos"][:max_number]
        all_labels[label_name]["neg"] = all_labels[label_name]["neg"][:max_number]

...
GEMMA4_MODELS = {
    'gemma-4-12b-it': {
        'tokenizer': {'path': 'google/gemma-4-12B-it'},
        'model': {
...
class Gemma4Model(VQAScoreModel):
    video_mode   = "direct"
    allows_image = True
    ...
    def __init__(self,
                 ...
                 cache_dir=None,
                 checkpoint=None):
        assert model_name in GEMMA4_MODELS, \
            f"Model {model_name} not found in GEMMA4_MODELS"
        self.model_name = model_name
        self.device     = device
        self.cache_dir  = cache_dir
        self.model_info = GEMMA4_MODELS[model_name]
    ...
    # ------------------------------------------------------------------
    # Frame extraction (same pattern as Gemma 3)
    # ------------------------------------------------------------------
    ...
    def load_images(self,
        ...
        """
        Load images or videos and return as Gemma 4 message-content lists.
    ...
    def _compute_token_prob(self,
        ...
        HF always receives temperature=1.0 so logits stay unscaled — we own
        the scaling here instead.
        """
    ...
    def _build_inputs(self, content: List[dict], question: str) -> dict:
        messages = [
            {
                "role": "user",
        ...
        inputs = self.processor.apply_chat_template(
            ...
            enable_thinking=False,   # disable thinking for VQAScore
        )
    ...
    def forward(self,
                ...
                max_new_tokens: int = 5,   # small buffer for empty thinking block
                temperature: float = 1.0) -> torch.Tensor:
        """
        Calculate VQAScore alignment scores.
        Temperature is applied manually — HF receives temperature=1.0.

        max_new_tokens=5 rather than 1 to handle Gemma 4's empty thinking
        block (<|channel>thought\\n<channel|>) which may precede the answer
            ...
            with torch.inference_mode():
                outputs = self.model.generate(
                    **inputs,
                    ...
                    temperature=1.0,   # HF must not apply temperature
                    do_sample=False,
                    output_scores=True,
    ...
    def generate(self,
        ...
        Uses processor.parse_response() to strip thinking tags from output.
        """
        assert len(images) == len(texts), "Number of images and texts must match"
        ...
        for content, text in zip(processed_data, texts):
            ...
            # skip_special_tokens=False required for parse_response to handle
            # thinking tags correctly
            raw_response = self.processor.decode(
                output_ids[0][in_len:], skip_special_tokens=False
            )
            parsed = self.processor.parse_response(raw_response)
            response = parsed['content'] if isinstance(parsed, dict) else parsed
